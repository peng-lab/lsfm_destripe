    ...
    def forward(self, X, y, r):
        ...
        for i, angle in enumerate(self.Angle):
            ...
            for index, i in enumerate(list_):
                ...
                bdetail.append(self.boxfilter(ydetail - Xdetail, weightDetail, self.KERNEL[0], self.PC[0], self.PR[0])[..., 5 if index != 0 else 0:-5 if index != len(list_)-1 else None])
                ...
                A = cov_xy / (var_x + 1e-6)
                b = mean_y - A * mean_x
                bbase.append(b[..., 5 if index != 0 else 0:-5 if index != len(list_)-1 else None])
                Abase.append(A[..., 5 if index != 0 else 0:-5 if index != len(list_)-1 else None])
