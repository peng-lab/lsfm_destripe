...
def fusion_perslice(GFbase, GFdetail, topSlice, bottomSlice, Gaussianr, kernel, boundary, device):
    topSlice = torch.from_numpy(topSlice).to(device)
    bottomSlice = torch.from_numpy(bottomSlice).to(device)
    topBase = torch.conv2d(F.pad(topSlice, (Gaussianr//2, Gaussianr//2, Gaussianr//2, Gaussianr//2), "reflect"), kernel)
    bottomBase = torch.conv2d(F.pad(bottomSlice, (Gaussianr//2, Gaussianr//2, Gaussianr//2, Gaussianr//2), "reflect"), kernel)
    ...
    result0detail, result1detail = GFdetail(bottomDetail, mask0), GFdetail(topDetail, mask1)
    ...
    minn, maxx = min(topSlice.min(), bottomSlice.min()), max(topSlice.max(), bottomSlice.max())
    result = torch.clip(result0base * bottomBase + result1base * topBase + result0detail * bottomDetail + result1detail * topDetail, minn, maxx)
...
def WedgeMask(md, nd, Angle, deg):
    ...
    if Angle != 0:tmp = rotate(tmp, Angle, reshape = False)
    ...
    return ((a <= math.pi/180*(90 - deg)).astype(np.int32) * (b > 18).astype(np.int32)) != 0
...
def prepare_aux(
    ...
    hier_ind: ndarray
        TODO
    """
    if is_vertical == False: (nd, md) = (md, nd)
    angleMask = np.stack([WedgeMask(md, nd, Angle = angle, deg = deg) for angle in angleOffset], 0)
